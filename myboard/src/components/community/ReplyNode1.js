...
const ReplyNode1 = (props) => {
    ...
    const replys_of_same_parent = useSelector(state => state.selling.replys);
    
    // state LV1 댓글중에 지금 현재 컴포넌트에 해당하는 댓글 찾는 'selector 함수'
    ...
    const clickHandler = (e) => {
        if(e.target.name === "reply_chk") {
            ...
            // });
            dispatch({
                type: "REPLY/RECURSIVELY_CHECK_UNCHECK_LV1",
                payload: {
                    LV1_IDX : props.idx,
        ...
        }else if(e.target.name === "reply_add") {
            setNewReply(true);
            ...
            if(window.confirm("선택하신 댓글을 포함해 하위 댓글이 모두 삭제됩니다.\n삭제하시겠습니까?")){
                dispatch(deleteReplyByOneCode(replyObj.sell_id, replyObj.reply_code, replyObj.level, props.idx, undefined, undefined, undefined));
            }
        }
    };
    ...
    return (
        <>
            <div className="div_replynode_LV1">
                ...
                <input type="checkbox" name="reply_chk" className="reply_chk_LV1" checked={replyObj.checked} value="1" onClick={clickHandler}/>
                <input type="button" name="reply_open_close" className="reply_open_close_LV1" value={openclose} onClick={clickHandler}/>
                ...
                <input type="button" name="reply_add" className="reply_add_LV1" value="답글 추가" onClick={clickHandler} />
                <input type="button" name="reply_del" className="reply_del_LV1" value="삭제" onClick={clickHandler} />
                <input type="button" name="reply_mod" className="reply_mod_LV1" value="수정" />
                <br/>
            </div>
            {openclose === "∨" ?
                replyObj.down_replys.map(
                    (e, idx) => <ReplyNode2 key={e.reply_code} LV1_IDX={props.idx} idx={idx} openclose={"＞"} {...e}/>
                )
                : null}
            {newReply ?
                <NewReply
